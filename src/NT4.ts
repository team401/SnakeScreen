...
export class NT4Client {
  ...
  private onNewTopicData: (topic: NT4Topic, timestamp_us: number, value: unknown) => void;
  ...
  constructor(
    ...
    onNewTopicData: (topic: NT4Topic, timestamp_us: number, value: unknown) => void,
    onConnect: () => void,
    onDisconnect: () => void
  ) {
  ...
  private ws_connect(rttWs = false) {
    ...
    if (!rttWs) {
      ...
      ws.addEventListener("error", (event: ErrorEvent) => this.ws_onError(ws));
