...
export class NT4Publisher {
  ...
  constructor(
    isSim: boolean,
    statusCallback: (status: NT4PublisherStatus) => void
  ) {
  ...
  private periodic() {
    ...
    let topicsToPublish = filterFieldByPrefixes(
      ...
      (topic) =>
        ...
        window.log.getType(topic) != LoggableType.Empty
    );
    ...
    Object.keys(this.publishedTopics).forEach((topic) => {
      ...
      let hasChanged =
        lastValue === null || !logValuesEqual(type, value, lastValue);
      if (hasChanged) {
