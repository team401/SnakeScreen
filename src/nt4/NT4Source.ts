...
export default class NT4Source extends LiveDataSource {
    ...
    this.periodicCallback = setInterval(() => {
      ...
      if (this.client !== null) {
        if (window.preferences?.liveSubscribeMode === "logging") {
          ...
          Object.values(this.lowBandwidthDataSubscriptions).forEach(
            (subscriptionId) => {
          ...
          if (this.lowBandwidthTopicSubscription === null) {
            this.lowBandwidthTopicSubscription =
              this.client.subscribeTopicsOnly(
                [this.akitMode ? AKIT_PREFIX + "/" : ""],
                true
              );
          }
          ...
          if (window.log === this.log) {
            let announcedKeys = this.log
              .getFieldKeys()
              .filter((key) => this.log?.getType(key) !== LoggableType.Empty);
          ...
          activeFields.forEach((field) => {
            ...
            if (!(field in this.lowBandwidthDataSubscriptions)) {
              ...
              this.lowBandwidthDataSubscriptions[field] = this.client.subscribe(
                [field],
                true,
                true,
                0.02
          ...
          Object.entries(this.lowBandwidthDataSubscriptions).forEach(
    ...
    if (window.preferences === null) {
      ...
      this.client = new NT4Client(
        ...
        (topic: NT4Topic, timestamp_us: number, value: unknown) => {
          ...
          let timestamp =
            Math.max(timestamp_us, this.log.getTimestampRange()[0]) / 1000000;
                ...
                if (topic.type.startsWith(STRUCT_PREFIX)) {
                  ...
                  if (CustomSchemas.has(topic.type)) {
                    try {
                      ...
                      console.error(
                        'Failed to decode custom schema "' + topic.type + '"'
                      );
                    }
  ...
  private getKeyFromTopic(topic: NT4Topic): string {
    if (this.akitMode) {
